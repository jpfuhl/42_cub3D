...
/*   Updated: 2022/07/14 15:10:56 by jpfuhl           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_frame(t_data *data, t_window *win, void *mlx)
{
	...
	create_image(data, &win->map, 225, 225);
	...
	draw_minimap(data, data->map, data->player, win->map);
	mlx_put_image_to_window(mlx, win->pointer, win->screen->pointer, 0, 0);
	mlx_put_image_to_window(mlx, win->pointer, win->map->pointer, 8, 8);
}
